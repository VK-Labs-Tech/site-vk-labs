...
export const Sistema2D = () => {
  return (
    <section id="sistema-2d" className="py-16 sm:py-24" aria-label="Sistema 2D para loja de calçados">
      <Container>
        ...
        <div className="grid items-start gap-12 lg:grid-cols-2">
          <div className="reveal rounded-3xl border border-warm/16 bg-gradient-to-br from-warm/[0.09] to-white/[0.025] p-6 shadow-panel">
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="m-0 text-[0.68rem] font-bold tracking-[0.16em] text-mute-dark uppercase">
                  Estoque em grade
                </p>
                <h3 className="mt-1 mb-0 text-xl font-bold">Tênis Runner Pro · Preto</h3>
              </div>
              <span className="rounded-full border border-warm/18 bg-warm/[0.05] px-3 py-1 text-xs font-semibold text-[#ffe0a4]">
                41 pares
              </span>
            </div>
            ...
            >
              {sizes.map((item) => (
                <div
                  key={item.n}
                  className={`rounded-lg border p-2 text-center transition duration-200 hover:-translate-y-0.5 ${
                    item.hot
                      ? 'border-success/30 bg-success/[0.075]'
                      : item.low
                        ? 'border-warm/30 bg-warm/[0.075]'
                        : 'border-white/[0.07] bg-black/16 hover:border-white/14'
                  }`}
                >
                  <span className="block text-xs text-[#8195a8]">{item.n}</span>
                  <strong className="text-sm text-white">{item.q}</strong>
                </div>
              ))}
            </div>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {features.map((feature, index) => (
              ...
              >
                <div className="mb-2 flex items-center gap-2 text-warm">
